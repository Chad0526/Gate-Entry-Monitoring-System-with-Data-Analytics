...
```
User (Admin / Faculty / Staff / Guard)
        ↓
Login & Role Check
        ↓
Dashboard
...
**Detailed flow (for thesis Chapter 3):** see **[SYSTEM_FLOW.md](SYSTEM_FLOW.md)**.  
...
| Area | Admin | Faculty | Staff | Guard |
|------|:-----:|:-------:|:-----:|:-----:|
| Dashboard | ✓ | ✓ | ✓ | ✓ |
| Events (create, list, categories, members) | ✓ | ✓ | ✓ | — |
| Gate scan, guard dashboard, entries, incidents, analytics | ✓ | — | ✓ | ✓ |
| Students (list, create, edit, import CSV) | ✓ | — | ✓ | — |
| Django admin site | ✓ | — | — | — |

Roles are enforced via Django Groups: **Admin**, **Faculty**, **Staff**, **Guard** (see `gate_analytics/roles.py`). Users must be in one of these groups (or have Staff/Superuser status) to log in.
...
- **Deny entry:** Deny gate entry, alert guard, record incident details (e.g. identity mismatch, invalid ID, not registered).
...
| URL | Description |
|-----|-------------|
| `/` | **Login page** (root; guards go to Gate scan after login) |
| `/login/` | Login (same as `/`) |
| `/dashboard/` | Dashboard (Admin / Faculty / Staff; guards redirect to Gate scan) |
| `/gate/` | **Gate scan** – scan QR or enter student ID (kiosk) |
...
| `/events/` | Event program schedule (create/list events) |
...
- **Flow:** After a successful QR scan at the gate, the guard (or student) can optionally use **Verify face (2FA)** in the popup: click the button → allow camera → capture → the captured face is compared to the student’s enrolled photo on file.
...
- At the gate: scan the QR (or type `student_id` manually) → system looks up the student → guard verifies identity → Grant or Deny.
...
4. Verify the profile and click **Grant entry** or **Deny entry** (if denied, incident is recorded and guard alerted).
