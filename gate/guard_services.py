...
from .models import (
    GuardNotification, GuardNote, GuardNoteRead, GuardActivityLog,
    GateEntry, GateIncident, GuardShift, Student, Event, EventAttendance
)
...
class GuardNotificationService:
    ...
    def create_incident_alert(incident, broadcast=True):
        ...
        Returns:
            GuardNotification object (or list if broadcast)
        """
    ...
    def create_capacity_alert(event, current_count, capacity):
        ...
        Returns:
            List of GuardNotification objects (broadcast to all on-duty guards)
        """
        if capacity <= 0:
            return []
    ...
    def create_shift_reminder(guard, shift, minutes_remaining=30):
        ...
        Args:
            ...
            shift: GuardShift object
            minutes_remaining: Minutes until shift ends
            
        Returns:
            GuardNotification object
        """
    ...
    def create_suspicious_activity_alert(details, broadcast=True):
        ...
        Returns:
            List of GuardNotification objects if broadcast, single object otherwise
        """
    ...
    def get_unread_notifications(guard):
        ...
        Returns:
            QuerySet of GuardNotification objects
        """
        now = timezone.now()
        return GuardNotification.objects.filter(
            target_guard=guard,
            is_read=False
        ).filter(
    ...
    def mark_as_read(notification_id, guard):
        ...
        Args:
            ...
            guard: User object (must be the target_guard)
            
        Returns:
            True if successful, False otherwise
        """
        try:
            notification = GuardNotification.objects.get(
                id=notification_id,
                target_guard=guard
            )
            ...
            return True
        except GuardNotification.DoesNotExist:
            return False
...
class GuardActivityLogger:
    ...
    def log_scan(guard, entry, device_id='', ip_address=None):
        ...
        log = GuardActivityLog.objects.create(
            guard=guard,
            action_type='scan',
    ...
    def get_guard_activity(guard, date_range=None):
        ...
        Returns:
            QuerySet of GuardActivityLog objects
        """
        logs = GuardActivityLog.objects.filter(guard=guard).order_by('-timestamp')
    ...
    def get_shift_activity(shift):
        ...
        """
        if shift.shift_end:
            return GuardActivityLog.objects.filter(
                guard=shift.guard,
...
class GuardPerformanceTracker:
    ...
    def get_shift_metrics(shift):
        ...
        incidents = GuardActivityLog.objects.filter(
            guard=shift.guard,
            action_type='incident',
            timestamp__gte=shift.shift_start,
            timestamp__lte=shift_end
        ...
        overrides = GuardActivityLog.objects.filter(
            guard=shift.guard,
            action_type='override',
            timestamp__gte=shift.shift_start,
            timestamp__lte=shift_end
    ...
    def get_incident_response_time(guard, date_range):
        ...
        incident_logs = GuardActivityLog.objects.filter(
            guard=guard,
            action_type='incident',
            timestamp__gte=start_date,
            timestamp__lt=end_date,
    ...
    def get_performance_summary(guard, period_start, period_end):
        ...
        avg_response_time = GuardPerformanceTracker.get_incident_response_time(guard, date_range)
...
class RealtimeDashboardService:
    ...
    def get_current_stats():
        ...
        return {
            ...
            'active_guards': GuardShift.objects.filter(shift_end__isnull=True).count(),
            'timestamp': now
        }
    ...
    def get_shift_summary(guard):
        ...
        """
        try:
            active_shift = GuardShift.objects.get(guard=guard, shift_end__isnull=True)
        ...
        # Get shift metrics
        metrics = GuardPerformanceTracker.get_shift_metrics(active_shift)
...
class GuardNotesManager:
    ...
    def create_note(guard, content, priority='normal', shift=None):
        ...
        Args:
            ...
            shift: GuardShift object (optional)
        ...
        # Log note creation
        GuardActivityLogger.log_note_creation(guard, note)
    ...
    def get_recent_notes(limit=10):
        ...
        Returns:
            QuerySet of GuardNote objects
        """
        return GuardNote.objects.select_related(
            'guard', 'shift'
    ...
    def get_unread_notes(guard, limit=10):
        ...
        read_note_ids = GuardNoteRead.objects.filter(
            guard=guard
        ).values_list('note_id', flat=True)
        
        return GuardNote.objects.exclude(
            id__in=read_note_ids
        ).exclude(
            guard=guard  # Exclude own notes
        ).select_related('guard', 'shift').order_by('-created_at')[:limit]
    ...
    def mark_note_read(note_id, guard):
        ...
        Args:
            note_id: ID of GuardNote
            guard: User object
            
        Returns:
            GuardNoteRead object or None if already read
        """
        try:
            note = GuardNote.objects.get(id=note_id)
        except GuardNote.DoesNotExist:
            return None
        
        # Create read record (unique constraint prevents duplicates)
        read_record, created = GuardNoteRead.objects.get_or_create(
            note=note,
            guard=guard
        )
    ...
    def search_notes(query='', date_range=None, priority=None, limit=50):
        ...
        Returns:
            QuerySet of GuardNote objects
        """
        notes = GuardNote.objects.select_related('guard', 'shift').order_by('-created_at')
...
class StudentLookupService:
    ...
    def get_current_schedule(student):
        """
        Get current class schedule for student from load slip.
        
        Args:
            student: Student object
            
        Returns:
            QuerySet of LoadSlipSubject objects or None
        """
        from .models import LoadSlip
        
        try:
            # Get active load slip
            load_slip = LoadSlip.objects.filter(
                student=student,
                status='approved'
            ).order_by('-created_at').first()
            
            if load_slip:
                return load_slip.subjects.all()
            return None
        except Exception:
            return None
    ...
    def get_today_schedule(student):
        ...
        """
        subjects = StudentLookupService.get_current_schedule(student)
        ...
        for subject in subjects:
            if subject.schedule:
                try:
                    schedule_data = json.loads(subject.schedule)
                    if day_name in schedule_data:
                        today_classes.append({
                            'subject': subject.subject_code,
                            'title': subject.subject_title,
                            'time': schedule_data[day_name].get('time', '—'),
                            'room': schedule_data[day_name].get('room', '—')
                        })
    ...
    def verify_class_time(student):
        """
        Verify if student has class at current time.
        ...
        for class_info in today_schedule:
            time_str = class_info.get('time', '')
            if time_str and time_str != '—':
                # Simple time range check (format: "08:00-09:00")
                if '-' in time_str:
                    start_time, end_time = time_str.split('-')
                    if start_time <= current_time <= end_time:
                        return {
                            'has_class': True,
                            'message': f"In class: {class_info['subject']}",
                            'class_info': class_info
                        }
        
        return {
            'has_class': False,
            'message': 'No class at this time'
        }
...
def check_event_capacity_and_alert(event):
    ...
    recent_alerts = GuardNotification.objects.filter(
        notification_type='capacity',
        related_event=event,
    ...
    alerts = GuardNotificationService.create_capacity_alert(
        event=event,
        current_count=current_count,
        capacity=event.maximum_attende
    )
