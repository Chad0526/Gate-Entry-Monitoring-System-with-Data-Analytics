#!/usr/bin/env python
"""
Verification script for GateNotification database setup.
...
from gate.models import GateNotification
...
def check_table_exists():
    """Check if GateNotification table exists."""
    print("\n" + "=" * 60)
    print("TABLE EXISTENCE CHECK")
    print("=" * 60)

    table_name = GateNotification._meta.db_table
...
def check_notification_count():
    ...
    try:
        total = GateNotification.objects.count()
        unread = GateNotification.objects.filter(is_read=False).count()
...
def create_test_notification():
    ...
    try:
        # Find a staff user (gate personnel at college)
        staff_users = User.objects.filter(groups__name__iexact='Staff')

        if not staff_users.exists():
            print("✗ No Staff group users found in the system")
            print("\nCreate a staff user first:")
            print("  1. Go to Django admin")
            print("  2. Create a user")
            print("  3. Add them to the 'Staff' group")
            return False

        personnel_user = staff_users.first()
        print(f"✓ Found staff user: {personnel_user.username}")
        ...
        print(f"  - Target: {notification.notify_user.username}")
        print(f"  - Created at: {notification.created_at}")

        # Verify it was saved to database
        saved = GateNotification.objects.filter(id=notification.id).exists()
        if saved:
...
def main():
    ...
    print("\n")
    print("╔" + "=" * 58 + "╗")
    print("║" + " " * 8 + "GATE NOTIFICATION DATABASE VERIFICATION" + " " * 9 + "║")
    print("╚" + "=" * 58 + "╝")
    print("\n")
